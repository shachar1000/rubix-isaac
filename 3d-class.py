...
class texturedCube():
    def __init__(self):
        self.images = [cv2.imread("{}.jpg".format(name))[0:360, 60:420] for name in ['cat', 'virus', 'hood']]
        ...
        for c, dicti in enumerate(average_z_list):
            ...
            if c > 2:
                if dicti["coor_no_change"] == 0: #x
                    cool_list.extend(texturedCube.transform(dicti, pointlist, 1, 2, True, True, self.images[dicti["coor_no_change"]]))
                elif dicti["coor_no_change"] == 1: #y
                    cool_list.extend(texturedCube.transform(dicti, pointlist, 2, 0, True, False, self.images[dicti["coor_no_change"]]))
                else: #z
                    # front 
                    cool_list.extend(texturedCube.transform(dicti, pointlist, 1, 0, True, False, self.images[dicti["coor_no_change"]]))
